...
1. 实现了SNI分流使用Local Loopback连接或使用Unix Donew Socket（UDS）连接及启用PROXY protocol支持。
2. 实现了反代使用Local Loopback连接或使用Unix Donew Socket（UDS）连接支持。
3. 实现了回落/分流使用Local Loopback连接或使用Unix Donew Socket（UDS）连接及启用PROXY protocol支持。
4. 实现了Caddy、V2Ray/Xray使用UDS连接时采用Abstract sockets模式（不需考虑权限问题）。
...
### 服务端单一/简单应用配置示例
#### &emsp;V2Ray/Xray的mKCP应用与Hysteria应用
...
### 服务端特殊应用配置示例
1. [V2Ray(Other Configuration)](https://github.com/lxhao61/integrated-examples/tree/new/V2Ray(Other%20Configuration)) （V2Ray或Xray的特色应用配置方法。）
